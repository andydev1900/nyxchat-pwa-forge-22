...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full px-4">
      {/* Logo Section - moved up and made bigger */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
        className="flex items-center justify-center pt-12 md:pt-20 pb-4 md:pb-8"
      >
      ...
      {/* Spacer to push buttons down more */}
      <div className="flex-1 min-h-[6rem] md:min-h-[4rem]"></div>

      {/* Quick Actions - moved down more and made slightly more squared */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, delay: 0.2 }}
        className="grid grid-cols-2 gap-3 md:gap-4 max-w-sm w-full mx-auto mb-32 md:mb-20"
      >
